...
samplePoints squareToCosineHemisphere(int sample_count){
    ...
    for (int t = 0; t < sample_side; t++) {
        for (int p = 0; p < sample_side; p++) {
            ...
            samlpeList.thetas.push_back(theta);
        }
    }
    return samlpeList;
}
...
Vec3f IntegrateEmu(Vec3f V, float roughness, float NdotV, Vec3f Ei) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        Eavg += Ei * sqrt(1 - NdotV * NdotV) * 2 / pdf;
    }
...
int main() {
	...
	else 
    {
        ...
        {
            float roughness = step * (static_cast<float>(i) + 0.5f); // 粗糙度
			for (int j = 0; j < resolution; j++) 
            {
                float NdotV = step * (static_cast<float>(j) + 0.5f);  // 法线与视角方向点积
