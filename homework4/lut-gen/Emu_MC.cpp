...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        Vec3f vi = sampleList.directions[i];  // 输入方向
        float pdf = sampleList.PDFs[i];
        Vec3f H = normalize(V + vi);
        float NoL = dot(vi, N);
        float D = DistributionGGX(N, H, roughness);
        float F = 1.0f;
        float G = GeometrySmith(roughness, NdotV, NoL);
        float brdf = D * F * G / (4 * NoL * NdotV);
        result += brdf * NoL / pdf;
    }

    return Vec3f(result / sample_count);
}
...
int main() {
    ...
    for (int i = 0; i < resolution; i++) {
        for (int j = 0; j < resolution; j++) {
            ...
            Vec3f irr = IntegrateBRDF(V, roughness, NdotV);  // 这是我们可以看到的irradiance， 粗糙度越低我们看到的越多，吸收的少
            //if (roughness < 0.4f)
                //std::cout << "roughness:" << roughness << "," << irr.x << std::endl;
    ...
    stbi_flip_vertically_on_write(true);  // 下半部分roughness从小到大
