...
async function GAMES202Main() {
	...
	{
		let Sphere0Transform = setTransform(step * space / 2 - space * i, 60, 0, 180, 180, 180, 0, Math.PI, 0);
		loadGLTF(renderer, 'assets/ball/', 'ball', 'KullaContyMaterial', Sphere0Transform, metallic, 1.0/7*i);
	}

	for (let i = 0; i < step; i++)
	{
		let Sphere0Transform = setTransform(step * space / 2 - space * i, -60, 0, 180, 180, 180, 0, Math.PI, 0);
		loadGLTF(renderer, 'assets/ball/', 'ball', 'PBRMaterial', Sphere0Transform, metallic, 1.0/7*i);
	}
